    ...
    public static void checkLatestLotInventory(){
        ...
        try {
            for (Product p : Product.values()) {
                ...
                if(null==lot){
                    System.out.println("null "+p.prefix);continue;
                }
...
//        test();
    ...
    static void test(){
        try {
            Product p=Product.ENA10;
            LotInfo lot = LotNumberUtil.getLatestLofInfoCount(p, true);
            if(lot.getAssembled()<10){
                Alert alert = new Alert(AlertType.INFORMATION);
                alert.setTitle("Plate inventory is low!");
                alert.setHeaderText(p.prefix+" in stock : "+lot.getAssembled());
                alert.setContentText(lot.getLotInfoLogEntry());
                alert.showAndWait();
            }else{
                System.out.println(lot.getLotInfoDbEntry());
            }
